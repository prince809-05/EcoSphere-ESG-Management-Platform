  ...
  return (
    <>
      <div className="lg:col-span-2 p-6 rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur-md flex flex-col justify-between">
        ...
        <div className="mt-6 pt-4 border-t border-slate-800/60 text-[10px] text-slate-500 flex items-center justify-between">
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 font-sans">
          <div className="w-full max-w-lg p-6 rounded-2xl bg-[#0d1017] border border-slate-800 shadow-2xl relative overflow-hidden">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                ...
                <span className="text-xs text-slate-400 font-semibold animate-pulse">Running ESG projection simulation...</span>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-850 text-xs text-slate-300 leading-relaxed whitespace-pre-line">
                  {roadmap}
                </div>
                <div className="flex justify-end pt-2">
                  <button
                    onClick={() => setModalOpen(false)}
                    className="px-4 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-xs font-semibold text-white border border-slate-800 transition-all"
                  >
                    Close Simulation
                  </button>
                </div>
